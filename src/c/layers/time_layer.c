...
void time_layer_create(Layer *parent_layer, GRect frame) {
  ...
  s_time_shadow_layer = text_layer_create(GRect(0, 0, frame.size.w, frame.size.h));
  ...
  layer_add_child(text_layer_get_layer(s_container_layer),
                  text_layer_get_layer(s_time_shadow_layer));
  layer_add_child(text_layer_get_layer(s_container_layer),
...
void time_layer_tick() {
  ...
  // Update shadow layer position (offset 3px right and down)
  text_layer_move_frame(s_time_shadow_layer,
                        GRect(text_left + SHADOW_OFFSET, text_top + SHADOW_OFFSET,
                              content_w, time_size.h));
...
void time_layer_refresh() {
  GFont font = config_time_font();
  text_layer_set_font(s_time_shadow_layer, font);
