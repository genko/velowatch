...
WeatherProvider.prototype.withCityName = function (lat, lon, callback) {
  // callback(cityName)
  var url =
    "https://geocode.arcgis.com/arcgis/rest/services/World/GeocodeServer/reverseGeocode?f=json&langCode=EN&location=" +
    lon +
    "," +
    lat;
...
WeatherProvider.prototype.withOpenHolidays = function (callback) {
  ...
  if (this.openHolidaysCountry === "") {
    ...
    openHolidays.getHolidayBitmask(
      this.openHolidaysCountry,
      this.openHolidaysRegional,
      function (bitmask) {
        callback(bitmask);
...
WeatherProvider.prototype.withProviderData = function (
  lat,
  lon,
  force,
  callback
    ...
    function (lat, lon) {
      this.withCityName(
        ...
        function (cityName) {
          this.withRiDuck(
            function (advice) {
              this.withOpenHolidays(
                function (holidays) {
                  this.withSunEvents(
                    lat,
                    lon,
                    function (sunEvents) {
                      this.withProviderData(
                        lat,
                        lon,
                        force,
                        function () {
...
WeatherProvider.prototype.hasValidData = function () {
  ...
  if (
    ...
    if (
      this.tempTrend.length >= this.numEntries &&
      this.precipTrend.length >= this.numEntries
    ) {
...
WeatherProvider.prototype.getPayload = function () {
  ...
  var temps = this.tempTrend
    .slice(0, this.numEntries)
  ...
  var daysIcons = this.daysIcon
    .slice(0, this.numDays)
    .map(function (temperature) {
      return temperature;
    });
  const precips = this.precipTrend
    .slice(0, this.numEntries)
    .map((p) => Math.round(p * 15));

  const precips_amount = this.precipMMH
    .slice(0, this.numEntries)
    .map(function (a) {
  ...
  var payload = {
    ...
    PRECIP_TREND_UINT8: Array.prototype.slice.call(combined),
    WINDSPEED_TREND_UINT8: Array.prototype.slice.call(windSpeeds),
