function request(url, type, callback, body, headers) {
...
RiDuckProvider.prototype.fetchAdvice = function (jwtToken, callback) {
  var url =
    "https://rest.riduck.com/json-api/dashboard.php?order=get_dashboard&days=84";
    ...
    function (xhr) {
      if (xhr.status === 200) {
        try {
          ...
          var advice_percentage = getIntervalPercentage(
            advice_range,
            current_tss
          );
          ...
          var adviceNumber = findStatus(advice);
